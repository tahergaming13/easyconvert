...
export function render(el: HTMLElement): void {
  ...
  el.innerHTML = `
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
      <div>
        <div id="dzText" class="dz cursor-pointer rounded-2xl border-2 border-dashed border-zinc-300 bg-zinc-50/60 p-5 text-center text-sm text-zinc-500 hover:border-blue-500 dark:border-zinc-700 dark:bg-zinc-900/50"><i class="ph ph-note-pencil text-xl text-blue-700 dark:text-blue-300" aria-hidden="true"></i> Drop a <b>.txt / .md</b> file here or <span class="font-semibold text-blue-700 dark:text-blue-300">click to browse</span><input id="txtFile" type="file" accept=".txt,.md,.markdown,text/plain" class="hidden" /></div>
      ...
      <div class="h-fit rounded-2xl border border-zinc-200 bg-zinc-100/70 p-5 dark:border-white/10 dark:bg-white/5">
        ...
        <button id="go" class="mt-5 w-full btn-press rounded-xl bg-zinc-950 px-4 py-3 text-sm font-bold text-white shadow-md transition hover:bg-zinc-800 disabled:opacity-40 dark:bg-white dark:text-zinc-950 dark:hover:bg-zinc-200">Create PDF <i class="ph ph-lightning text-base" aria-hidden="true"></i></button>
        ${progressBarHTML()}
      </div>
    </div>`
